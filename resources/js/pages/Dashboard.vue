  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
      <div class="grid auto-rows-min gap-4 md:grid-cols-3">
        ...
        <div class="flex justify-center relative aspect-video overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
        ...
        <div class="relative aspect-video overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
          <a href="/products" class="flex items-center justify-center text-white bg-black w-full h-full text-4xl">Epood</a>
        </div>
      </div>
    ...
    <Dialog v-model:open="showAddDialog">
        <DialogContent>
            ...
            <div>
                ...
                <input :value="newMarker.latitude" disabled class="w-full rounded border p-2 bg-gray-100" />
            </div>
            <div>
                <label class="text-sm font-medium">Longitude</label>
                <input :value="newMarker.longitude" disabled class="w-full rounded border p-2 bg-gray-100" />
            </div>
            </div>

            <DialogFooter>
            <button @click="saveMarker" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Save Marker</button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
    ...
    <Dialog v-model:open="showEditDialog">
        <DialogContent>
            ...
            <div>
                <label class="text-sm font-medium">Latitude</label>
                <input :value="editableMarker.latitude" disabled class="w-full rounded border p-2 bg-gray-100" />
            </div>
            <div>
                <label class="text-sm font-medium">Longitude</label>
                <input :value="editableMarker.longitude" disabled class="w-full rounded border p-2 bg-gray-100" />
            </div>
            </div>

            <DialogFooter>
            <button @click="updateMarker" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Save Changes</button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
